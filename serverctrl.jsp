...
//                fw.close();
        ...
        if(request.getParameter("process").compareTo("startserver") == 0) {
            try {
                Runtime.getRuntime().exec("chmod 777 /home/server");
                File workingDirectory = new File("/home");

                Runnable startServerTask = () -> {
                    try {
                        ProcessBuilder builder = new ProcessBuilder("/bin/sh", "-c", "/home/./server start");
...
fw.write("sync");
    ...
    else
    {
        String[][] m =
        {
            ...
            {"0", "is74", "heavenfall temple"},
            {"0", "is75", "heavenfall temple"},
            {"0", "random03", "quicksand maze"},
            {"0", "random04", "quicksand maze (?) (instance)"},
            {"0", "bg01", "Territory War T-3 PvP"},
            {"0", "bg02", "Territory War T-3 PvE"},
            {"0", "bg03", "Territory War T-2 PvP"},
            {"0", "bg04", "Territory War T-2 PvE"},
		...
		<tr>
			<td align="center">
				...
				<%
					for(int i=0; i<maps.length; i++)
					{
						if(maps[i][0].compareTo("0") != 0 && !maps[i][0].equals("ERROR"))
						{
