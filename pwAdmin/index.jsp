...
<body>
    <%
		boolean enableIPWhitelist = enable_ip_whitelist; // Set based on the config
       ...
       if(enableIPWhitelist)
       {
        try {
            ...
            out.println("<div style=\"color:red;text-align:center;font-size:2em;margin-top:5em;\">Error reading whitelist file.</div>");
            return; // Stop processing the page
         }
		} else {
			ipAllowed = true; // If whitelisting is disabled, allow all IPs
		}



        if (!ipAllowed) {
            out.println("<div style=\"color:red;text-align:center;font-size:2em;margin-top:5em;\">Access Denied - Your IP (" + clientIP + ") is not on the whitelist.</div>");
            return;
        }
    ...
    <%
        if(isLoggedIn)
		{
			if (request.getParameter("page") != null) {
				...
				if(pageParam.compareTo("account") == 0)
				{
                    pageContext.include("account.jsp");
				}
				else if(pageParam.compareTo("role") == 0)
				{
                    pageContext.include("role.jsp");
				}
                else if(pageParam.compareTo("rolexml") == 0)
				{
					pageContext.include("rolexml.jsp");
                }
                 else if(pageParam.compareTo("server") == 0)
				{
                    pageContext.include("server.jsp");
				}
                 else if(pageParam.compareTo("serverctrl") == 0)
				{
                    pageContext.include("serverctrl.jsp");
				}
			}
			else if(request.getParameter("plugin") != null)
			{
    ...
    <div id="loginOverlay"  <% if(isLoggedIn){ %>style="display:none; pointer-events:none;"<%}%>>
        <div id="loginBox">
